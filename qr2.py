...
def find_corresponding_left_image(qr_data, force_reload=False):
    ...
    try:
        ...
        # 方法1: ファイル名から直接QR番号を抽出（新しい命名規則）
        qr_pattern = f"QR{qr_number:03d}"
        for filename in left_video_files:
            if qr_pattern in filename:
        ...
        # 方法2: ファイル名からpause_sequence_counterを抽出（従来の方法）
        for filename in left_video_files:
            try:
...
def save_qr_detection_images(frame, detected_qr_positions, frame_count, output_frame=None, increment_pause=True, force_save_preview=False, save_preview=True, save_left_video=True):
    ...
    # ファイル名を生成（QR番号を含む確実な命名規則）
    current_time = datetime.now()
    time_str = current_time.strftime("%Y%m%d_%H%M%S")
    detection_count = len(detected_qr_positions)
    
    # 現在表示中のQRコードの番号を取得
    current_qr_number = None
    if current_display_qr and current_display_qr in qr_manager:
        current_qr_number = qr_manager[current_display_qr]
    
    # QR番号を含むファイル名を生成
    if current_qr_number:
        base_filename = f"capture_{pause_sequence_counter:06d}_{time_str}_frame{frame_count}_{detection_count}qr_codes_QR{current_qr_number:03d}"
    else:
...
print("  '2'キー: 内容プレビューを手動保存")
print("  'Enter'キー: 現在表示中のURLをブラウザで開く")
print("  '3-9'キー: 履歴から直接選択")
print("  'q'キー: 終了")
print("色分け表示:")
print("  赤色: 新しく検出されたQRコード")
print("  シアン色: 現在選択中のQRコード")
print("  緑色: プレビュー保存済みのQRコード")
...
while True:
    ...
    available_height = WINDOW_HEIGHT - counter_y - 20  # 残りの高さ（下部のマージンを含む）
    ...
    for i, qr_data in enumerate(display_history):
        # 画面からはみ出す場合はスキップ
        if y_offset + dynamic_spacing > WINDOW_HEIGHT - 10:
            break
