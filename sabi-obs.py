...
def analyze_rust_area_detailed(frame, plate_detections):
    ...
    for i, plate_detection in enumerate(plate_detections):
        ...
        # 錆っぽい色範囲でマスクを作成（明度のみを条件に）
        # 明度(V): 15-86（暗い色をサビとして認識）
        v_condition = (hsv[:,:,2] >= 15) & (hsv[:,:,2] <= 120)  # 明度は15-86（暗い色）
...
def save_detection_images(frame, detections_with_confidence, frame_count, leftup_info, leftdown_info, rust_analysis, save_leftup=False, save_leftdown=False, leftup_image=None, leftdown_image=None):
    """一時停止時に検出画像を保存する関数（左上、左下、右上の3枚の画像を保存）"""
    ...
    # 左上画像：錆分析画像を保存（save_leftup=Trueの場合のみ）
    if save_leftup:
        if leftup_image is not None:
            try:
                ...
                # 錆をマークした画像を作成
                marked_image = mark_rust_on_detected_image(resized_img, leftup_info, save_size, save_size)
                
                # 左上画像を保存
                leftup_filepath = os.path.join(session_folder, f"{base_filename}_M_leftup_original.jpg")
                cv2.imwrite(leftup_filepath, marked_image)
                print(f"左上画像（錆分析）を保存しました: {base_filename}_M_leftup_original.jpg")
            ...
            leftup_filepath = os.path.join(session_folder, f"{base_filename}_M_leftup_original_no_target.jpg")
            empty_img = np.zeros((512, 512, 3), dtype=np.uint8)
            cv2.imwrite(leftup_filepath, empty_img)
            print(f"左上画像（No Target）を保存しました: {base_filename}_M_leftup_original_no_target.jpg")
            image_save_counter += 1
    
    # 左下画像：錆マスク画像を保存（save_leftup=Trueの場合のみ）
    if save_leftup:
        try:
            # 左下パネル用の画像を作成
            ...
            if leftdown_rust_mask is not None:
                ...
                panel_ld = cv2.resize(rust_mask_color, (save_size, save_size))
                ...
                panel_ld = np.zeros((save_size, save_size, 3), dtype=np.uint8)
                cv2.putText(panel_ld, 'No Rust Mask Data', (20, save_size//2), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)
            
            # 左下画像を保存
            leftdown_filepath = os.path.join(session_folder, f"{base_filename}_M_leftdown.jpg")
            cv2.imwrite(leftdown_filepath, panel_ld)
            print(f"左下画像（錆マスク）を保存しました: {base_filename}_M_leftdown.jpg")
            image_save_counter += 1
            
        except Exception as e:
            print(f"左下画像の保存に失敗しました: {e}")
    
    # 右上画像：検出中の画像（YOLOバウンディングボックス表示）を保存（save_leftup=Trueの場合のみ）
    if save_leftup:
        try:
            # 全体画像：全ての検出物体にバウンディングボックスを付けた画像を作成
            ...
            # 右上画像を保存
            rightup_filepath = os.path.join(session_folder, f"{base_filename}_R_whole_detection.jpg")
            cv2.imwrite(rightup_filepath, annotated_img)
            print(f"右上画像（検出結果）を保存しました: {base_filename}_R_whole_detection.jpg (検出物体数: {detection_count})")
            image_save_counter += 1
...
while True:
    ...
    if detections_with_confidence:
        ...
        total_rust_area = rust_analysis.get('total_rust_area', 0)
        
        for detail in rust_analysis.get('rust_details', []):
            try:
                rust_mask = detail.get('rust_mask')
        ...
        if small_count + large_count > 0:
            info_lines.append(f'Size: S:{small_count} L:{large_count}')
    ...
    if key == ord('q'):
        ...
        if leftup_paused:
            ...
            save_detection_images(frame, detections_with_confidence, leftup_frame_count, leftup_info, leftdown_rust_info, rust_analysis, save_leftup=True, save_leftdown=True, leftup_image=leftup_image, leftdown_image=None)
