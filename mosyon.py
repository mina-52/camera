...
model = YOLO('morsyon.pt')  # target2.ptファイルを使用

print("="*60)
print("リアルタイム検出を開始します（全画面モード）")
print("="*60)
print("キー操作:")
print("  '1'キー: 左上を一時停止/再開")
print("  '2'キー: 左下を一時停止/再開")
print("  'Enter'キー: 右上（検出画面）の全画面表示/四分割表示を切り替え")
print("  'q'キー: 終了")
print("="*60)
print("📌 注意: このモードでは画像保存は行いません")
print("="*60)
...
fullscreen_mode = True  # False: 四分割表示、True: 右上全画面表示（最初から全画面）
...
while True:
    ...
    if fullscreen_mode:
        ...
        # 操作ヒントを表示
        cv2.putText(combined, "1=Pause/Resume Left-Up  |  2=Pause/Resume Left-Down  |  q=Quit", 
                   (40, window_height - 120), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (200, 200, 200), 2)
        cv2.putText(combined, "Press Enter to switch to 4-split view", (40, window_height - 40), 
