    ...
    if save_leftup:
        ...
        if leftup_image is not None:
            ...
            if leftup_info:
                ...
                # デバッグ情報を出力
                print(f"左上パネル - 元座標: ({x1},{y1},{x2},{y2}), パネル座標: ({panel_x1},{panel_y1},{panel_x2},{panel_y2}), スケール: ({scale_x:.3f},{scale_y:.3f})")
    ...
    if save_leftdown:
        ...
        if leftdown_image is not None:
            ...
            if leftdown_info:
                ...
                # デバッグ情報を出力
                print(f"左下パネル - 元座標: ({x1},{y1},{x2},{y2}), パネル座標: ({panel_x1},{panel_y1},{panel_x2},{panel_y2}), スケール: ({scale_x:.3f},{scale_y:.3f})")
...
while True:
    ...
    info_lines.append(f'Pause Count: {pause_counter}')  # 一時停止回数
    info_lines.append(f'Image Saves: {image_save_counter}')  # 画像保存回数
    ...
    if key == ord('q'):
        ...
        if leftup_paused:
            print("左上を一時停止しました")
            pause_counter += 1  # 一時停止回数をカウント
            # 左上一時停止時に画像を保存（左上パネルのみ）
